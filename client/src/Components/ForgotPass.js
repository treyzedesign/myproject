...
const ForgotPass = () => {
    ...
    const forPassword = async()=>{
        ...
        await axios.post('http://localhost:3001/api/v1/signup/password', data).then((feedback)=>{
          ...
          setinfo("email has been sent to you")
        }).catch((fail)=>{
          console.log(fail);
          setinfo("invalid credentials or server error")
        })
    }
  return (
    <div>
        ...
        <div className='user-formlog  px-5 py-3 shadow-lg'>
          <form className="form-signin">
            ...
            <small className='text-muted'>Please enter the email address registered with your account.</small><br/>
            <small className='text-black'>{info}</small>
